...
bool Sprite::Init(std::wstring fileName)
{
	...
	if (m_Texture)
	{
		...
		m_Center = D3DXVECTOR3(m_Size.x / 2, m_Size.y / 2, 0);
		return true;
	}
	else
		return false;
...
void Sprite::Render()
{
	...
	if (m_Visible == false)
		m_pSp->Draw(m_Texture->GetTexture(), &m_Rect, &m_Center, 0, D3DCOLOR_ARGB(0, R, G, B));
	else
		m_pSp->Draw(m_Texture->GetTexture(), &m_Rect, &m_Center, 0, D3DCOLOR_ARGB(A, R, G, B));
